...
class DataManager:
    ...
    def fetchall(self, table_name: str, columns: List[str] = None) -> Tuple[bool, List[Dict[str, Any]]]:
        """
        Fetch all records from a table with optional column selection.
        
        This is a convenience method that provides a simpler interface than select()
        when you want to retrieve all records without conditions.
        
        Args:
            table_name (str): The name of the table to fetch from
            columns (List[str], optional): Specific columns to retrieve. Defaults to all columns.
            
        Returns:
            Tuple[bool, List[Dict[str, Any]]]: A tuple containing:
                - Success flag (bool)
                - List of records as dictionaries if successful, or error message string if failed
        """
        if not self.conn_manager.is_connected():
            return False, "Not connected to any database"
        ...
        # Validate column names if specified
        if columns and not self.schema_manager.validate_column_names(table_name, columns):
            return False, "Invalid column names in fetchall"
        
        try:
            # Use existing select method for implementation
            records = self.select(table_name, columns=columns)
            return True, records
        except Exception as e:
            logger.error(f"Error in fetchall: {e}")
            return False, f"Error fetching records: {e}"